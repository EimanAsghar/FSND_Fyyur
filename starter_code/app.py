...
def venues():
    ...
    # RESOURCE: https://pylidc.github.io/tuts/advanced-queries.html and https://www.youtube.com/watch?v=iDrs1S_ySg4
    data = []
    #group by is used to group venues by their state + city  
    aggregations =  db.session.query(Venue.state, Venue.city).group_by(Venue.state, Venue.city).all()
    
    try:
        for aggr in aggregations: #for every group walkthrough its venues 
            state = aggr[0]
            city = aggr[1]
            venues = db.session.query(Venue).filter_by(state=state, city=city).all()#feach venue
            venue_list = []# inilized here to reset for every group
            for v in venues:  
                    venue_list.append({
                    'id': v.id,
                    'name': v.name,
...
#     result = Venue.query.with_entities(
